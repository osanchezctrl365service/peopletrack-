...
// ─── POST /candidates/:id/retrocede (volver a etapa anterior) ──
app.http('retroceCandidate', {
  methods: ['POST'], authLevel: 'anonymous', route: 'candidates/{id}/retrocede',
  handler: async (req) => {
    try {
      const id = parseInt(req.params.id);
      const b = await req.json();
      ...
      if (!current.recordset.length) return err('Not found', 404);
      const oldStatus = current.recordset[0].Status;
      // Mapa inverso del advance
      const prevMap = { hired:'offer', offer:'client', client:'technical', technical:'screening' };
      const newStatus = prevMap[oldStatus] || oldStatus;
      if (newStatus === oldStatus) {
        return ok({ status: oldStatus, message: 'Ya está en la primera etapa' });
      }
      await query(`UPDATE Candidates SET Status=@s WHERE CandidateID=@id`, { s: newStatus, id });
      await query(
        `INSERT INTO CandidateHistory (CandidateID, ChangedByID, OldStatus, NewStatus, Note)
         VALUES (@cid, @by, @old, @new, @note)`,
        { cid: id, by: b.changedById||1, old: oldStatus, new: newStatus,
          note: b.note || 'Retrocedido a etapa anterior' }
      );
      return ok({ oldStatus, newStatus });
    } catch(e) { return err(e.message, 500); }
  }
...
app.http('candidateNotes', {
  ...
  handler: async (req) => {
    try {
      const id = parseInt(req.params.id);
      const b  = await req.json();
      // 1) Actualizar notas de la etapa actual
      await query(
        `UPDATE Candidates SET Notes=@notes WHERE CandidateID=@id`,
        { notes: b.notes||null, id }
      );
      // 2) Si hay datos de entrevista, registrarla. Si falla, propagar el error.
      let interviewSaved = false;
      let interviewError = null;
      if (b.interviewer || b.result || b.feedback) {
        try {
          // InterviewType es NOT NULL en la tabla → derivamos del Status del candidato
          // si el frontend no lo manda explícitamente
          const stage = b.stage || 'general';
          await query(
            `INSERT INTO CandidateInterviews
               (CandidateID, InterviewType, InterviewDate, InterviewerName, Result, Feedback, CreatedAt)
             VALUES (@cid, @itype, GETDATE(), @interv, @result, @feed, GETDATE())`,
            { cid: id, itype: stage,
              interv: b.interviewer||null, result: b.result||null, feed: b.feedback||null }
          );
          interviewSaved = true;
        } catch(e2) {
          interviewError = e2.message;
        }
      }
      // Si pidió guardar entrevista pero falló, devolver error claro
      if ((b.interviewer || b.result || b.feedback) && !interviewSaved) {
        return err('Notas guardadas pero la entrevista falló: ' + interviewError, 500);
      }
      return ok({ updated: true, interviewSaved });
    } catch(e) { return err(e.message, 500); }
  }
});
